...
| Онбординг | **Сейчас:** один экран `/onboarding` с вкладками «Новый seed» / «Импорт». **Целевое ([[12-Журнал-решений#DR-014]]):** после login — если vault на сервере есть → **только импорт** seed+KDF; если vault нет → **только генерация** + ack DR-006. Пароль KDF должен совпадать с тем, что при создании vault. При `unlocked` в браузере — сразу `/app`. |
