...
|------------------------|---------------------|
| Сборка, **SPA**, редиректы для клиентского роутера на Vercel | Полноценный **PWA** (SW, manifest — включить следующим коммитом) |
...
| `/onboarding` | Генерация seed / пароль для PBKDF2 ([[12-Журнал-решений#DR-005]]) |
...
1. Vite PWA plugin + manifest + offline shell.
2. TanStack Query + явная синхронизация vault с индикатором конфликта.
3. Wizard приоритетов и поля задачи из MVP.
4. End-of-day и отчёты на клиенте по [[12-Журнал-решений]].
