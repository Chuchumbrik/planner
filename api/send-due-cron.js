/**
 * Триггер для Supabase Edge `send-due`: Vercel Cron (GET) или внешний планировщик (POST).
 *
...
module.exports = async (req, res) => {
  ...
  const url = process.env.SUPABASE_SEND_DUE_URL
  const anon = process.env.SUPABASE_CRON_ANON_KEY
  if (!url || !anon) {
    res.status(500).json({
      error: 'missing_env',
      hint: 'Set SUPABASE_SEND_DUE_URL and SUPABASE_CRON_ANON_KEY on Vercel',
    })
    return
  }
